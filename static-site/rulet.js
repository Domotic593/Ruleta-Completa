...
class RuletaGame {
    ...
    async cargarDatosUsuario() {
        try {
            ...
            if (puntosGuardados) {
                this.puntos = parseInt(puntosGuardados);
            }
        } catch (error) {
            console.log('Usuario nuevo, empezando con 100 puntos');
        }
    }
    ...
    getDefaultItems() {
        return [
            { text: "Premio Mayor", type: "prize", puntos: 100, color: "#FFD700" },
            { text: "Viaje a la Playa", type: "prize", puntos: 80, color: "#4CAF50" },
            { text: "Tarjeta Regalo $50", type: "prize", puntos: 60, color: "#2196F3" },
            { text: "Pierdes Turno", type: "penalty", puntos: -10, color: "#F44336" },
            { text: "Giro Extra", type: "bonus", puntos: 0, color: "#9C27B0" },
        ];
    }
    ...
    showResult(data) {
        ...
        // Efecto de confeti para premios
        if (resultado.type === 'prize' || resultado.type === 'wildcard') {
    ...
    playConfettiEffect() {
        // Efecto visual simple de confeti
        console.log('🎉 ¡Efecto de confeti!');
        // Aquí podrías integrar una librería de confeti como canvas-confetti
    }
}
